...
module.exports = function(){
    ...
    this.When(/^I search for "([^"]*)"$/, async function(searchPhrase){
        ...
        //  let result = (await $('.elm-product-list-item-full-info')).length;
        //  await sleep(4*s);
        //  console.log("*** " + result);
        //  assert(result / 1 > 0, "The product is not found");
    });

    
    this.Then(/^the products text contains "([^"]*)"$/, async function (description){
    ...
    this.Then(/^confirm that is open from "([^"]*)" to "([^"]*)" on "([^"]*)"$/, async function (h1,h2,d){
        let moreBtn = await $('.sb-btn.outlined.full-width');
        await sleep(s);
        await moreBtn.click();
        await sleep(s);
    ...
    this.Then(/^enter the shop "([^"]*)"$/, async function (shop){
        ...
        await sleep(3 *s);
        //Select the shop
        obj = await $('.name.combined-match.ng-binding.ng-scope');
        await obj.click();
        await sleep(4 *s);
    });
